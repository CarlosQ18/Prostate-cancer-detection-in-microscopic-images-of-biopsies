...
for i = 1:length(XTestFinal)
    Prediccion = predict(NNModel, XTestFinal(i,:));
    if string(Prediccion) == 'G345'
        Prediccion = predict(NNModel2, XTestFinal(i,:));
        if string(Prediccion) == 'G45'
            Prediccion = predict(NNModel3, XTestFinal(i,:));
            YPredFinal = [YPredFinal;Prediccion];
        else
            YPredFinal = [YPredFinal;Prediccion];
        end
    else
...
title = "Clasificación en Cascada con NN " + string(accuracy);
