...
SVMModel = fitcsvm(XTrain, Y_TRAIN, 'ClassNames',classNames,'KernelFunction','linear','Prior', priorProb);
...
title = "Clasificación Binaria Grado 0 vs Grado 3-4-5 SVM " + string(accuracy);
figure
...
SVMModel2 = fitcsvm(XTrain2, Y_TRAIN2, 'ClassNames',classNames,'KernelFunction','linear','Prior', priorProb);

% Realizar predicciones en los datos de prueba
YPred2 = predict(SVMModel2, XTest2);
...
title = "Clasificación Binaria Grado 3 vs Grado 4-5 SVM " + string(accuracy);
figure
...
SVMModel3 = fitcsvm(XTrain3, Y_TRAIN3, 'ClassNames',classNames,'KernelFunction','linear','Prior', priorProb);
% Realizar predicciones en los datos de prueba
YPred3 = predict(SVMModel3, XTest3);
...
title = "Clasificación Binaria Grado 4 vs Grado 5 SVM " + string(accuracy);
...
XTestFinal = [XTest(1:data_test,:);XTest2(1:data_test,:);XTest3(:,:)];
YTestFinal = [Y_TEST(1:data_test,:);Y_TEST2(1:data_test,:);Y_TEST3(:,:)];
YPredFinal = [];

lengthFinal = length(XTestFinal);
for i = 1:lengthFinal
    Grado = predict(SVMModel, XTestFinal(i,:));
    if string(Grado) == "G345"
        Grado = predict(SVMModel2, XTestFinal(i,:));
        if string(Grado) == "G45"
            Grado = predict(SVMModel3, XTestFinal(i,:));
            YPredFinal = [YPredFinal;Grado];
        else
            YPredFinal = [YPredFinal;Grado];
        end
    else
...
CFinal = confusionmat(YTestFinal, YPredFinal);

% Calcular las métricas de desempeño
accuracy = sum(diag(CFinal)) / sum(CFinal(:));

title = "Clasificación Final NN " + string(accuracy);

YTestFinal = cellstr(YTestFinal);

figure
cmfinal = confusionchart(YTestFinal,YPredFinal, ...
    'ColumnSummary','column-normalized', ...
    'RowSummary','row-normalized',...
    'Title',title);