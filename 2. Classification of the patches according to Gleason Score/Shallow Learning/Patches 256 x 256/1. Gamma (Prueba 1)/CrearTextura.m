...
Name = 'Gamma_GLCMstats_'+ Tipo +'.txt';
...
for i = 1:Clases
    ...
    for j = 1:numel(Image_names)
        ...
        if isempty(Indice)
            ...
            I = rgb2gray(I);
            gamma = 1;
            imagegam = (255/(255^gamma)) * (double(I).^gamma);
            imagegam = uint8(imagegam);
            % debe tenerse en cuenta el rango dinámico de la nueva imagen
            limites = stretchlim(imagegam);
            imagegamn = imadjust(imagegam,limites,[]);
            GLCM2 = graycomatrix(imagegamn,'Offset',[2 0;0 2]);
