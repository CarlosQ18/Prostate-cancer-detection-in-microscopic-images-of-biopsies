...
%Base de Datos
rng('default');
Stats_path = fullfile(pwd,'Contourstats_Train.txt');
...
Stats_table = table2array(Stats_table(:,2:end));
G0 = Stats_table(Stats_table(:,end) == 0,1:end-1);
G3 = Stats_table(Stats_table(:,end) == 3,1:end-1);
G4 = Stats_table(Stats_table(:,end) == 4,1:end-1);
G5 = Stats_table(Stats_table(:,end) == 5,1:end-1);

G0 = G0(randi([1 size(G0,1)],1,5000) ,:);
G3 = G3(randi([1 size(G3,1)],1,5000) ,:);
G4 = G4(randi([1 size(G4,1)],1,5000) ,:);
G5 = G5(randi([1 size(G5,1)],1,5000) ,:);
...
results = fitcnb(XTrain, Y_TRAIN, 'ClassNames',classNames,'OptimizeHyperparameters','all',...
...
Distrib = results.DistributionNames;
Wdth = results.Width;
krnl = results.Kernel;

if isempty(krnl{1,1}) && isnan(Wdth(1,1))
    NBModel = fitcnb(XTrain, Y_TRAIN, 'ClassNames',classNames, 'DistributionNames',Distrib,'Prior', priorProb);
else
    NBModel = fitcnb(XTrain, Y_TRAIN, 'ClassNames',classNames, 'DistributionNames',Distrib,'Width',Wdth,'Kernel',krnl,'Prior', priorProb);
end
...
title = "Clasificación Binaria Grado 0 vs Grado 3-4-5 Bayes " + string(accuracy);
figure
...
results = fitcnb(XTrain2, Y_TRAIN2, 'ClassNames',classNames,'OptimizeHyperparameters','all',...
    'HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus','ShowPlots',false,'Verbose',0,'Kfold',Kfld));
...
krnl = results.Kernel;
if isempty(krnl{1,1}) && isnan(Wdth(1,1))
    NBModel2 = fitcnb(XTrain2, Y_TRAIN2, 'ClassNames',classNames, 'DistributionNames',Distrib,'Prior', priorProb);
else
    NBModel2 = fitcnb(XTrain2, Y_TRAIN2, 'ClassNames',classNames, 'DistributionNames',Distrib,'Width',Wdth,'Kernel',krnl,'Prior', priorProb);
end


% Realizar predicciones en los datos de prueba
YPred2 = predict(NBModel2, XTest2);
...
title = "Clasificación Binaria Grado 3 vs Grado 4-5 Bayes " + string(accuracy);
figure
...
results = fitcnb(XTrain3, Y_TRAIN3, 'ClassNames',classNames,'OptimizeHyperparameters','all',...
    'HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus','ShowPlots',false,'Verbose',0,'Kfold',Kfld));
...
if isempty(krnl{1,1}) && isnan(Wdth(1,1))
    NBModel3 = fitcnb(XTrain3, Y_TRAIN3, 'ClassNames',classNames, 'DistributionNames',Distrib,'Prior', priorProb);
else
    NBModel3 = fitcnb(XTrain3, Y_TRAIN3, 'ClassNames',classNames, 'DistributionNames',Distrib,'Width',Wdth,'Kernel',krnl,'Prior', priorProb);
end

% Realizar predicciones en los datos de prueba
YPred3 = predict(NBModel3, XTest3);
C3 = confusionmat(Y_TEST3, YPred3);

% Calcular las métricas de desempeño
accuracy = sum(diag(C3)) / sum(C3(:));

title = "Clasificación Binaria Grado 4 vs Grado 5 Bayes " + string(accuracy);
